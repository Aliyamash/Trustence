...
function CreatePanel({ type, item, apiKey, onClose, onCreated }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      const body = isProject
        ? { title: form.get("title"), category_name: form.get("category_name"), intro: form.get("intro"), description: form.get("description"), link: form.get("link"), tags: form.get("tags"), banner: imagePath, is_published: form.get("is_published") === "on" }
        : { name: form.get("name"), position: form.get("position"), bio: form.get("bio"), github: form.get("github"), twitter: form.get("twitter"), linkedin: form.get("linkedin"), sort_order: Number(form.get("sort_order") || 0), profile: imagePath, is_published: form.get("is_published") === "on" };
      const endpoint = isProject
        ? (isEditing ? `/admin/projects/${item.id}` : "/admin/projects")
        : (isEditing ? `/admin/team/${item.id}` : "/admin/team");
      const saved = await apiRequest(endpoint, apiKey, { method: isEditing ? "PUT" : "POST", body: JSON.stringify(body) });
      if (isProject) {
    ...
    <div className="fixed inset-0 z-[130] flex items-end justify-center bg-slate-950/45 p-0 backdrop-blur-sm sm:items-center sm:p-5" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <form onSubmit={submit} className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-t-[2rem] bg-white p-6 shadow-2xl sm:rounded-[2rem] sm:p-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {isProject ? <><input className={inputClass} name="title" placeholder="عنوان پروژه" defaultValue={item?.title || ""} required /><input className={inputClass} name="category_name" placeholder="دسته‌بندی" defaultValue={item?.category_name || ""} required /><textarea className={`${inputClass} sm:col-span-2`} name="intro" placeholder="معرفی کوتاه" defaultValue={item?.intro || ""} required /><textarea className={`${inputClass} min-h-36 sm:col-span-2`} name="description" placeholder="توضیحات کامل" defaultValue={item?.description || ""} /><input className={inputClass} name="tags" placeholder="تگ‌ها با کاما" defaultValue={item?.tags || ""} /><input className={inputClass} name="link" type="url" placeholder="https://project.example" defaultValue={item?.link || ""} /></> : <><input className={inputClass} name="name" placeholder="نام کامل" defaultValue={item?.name || ""} required /><input className={inputClass} name="position" placeholder="سمت" defaultValue={item?.position || ""} required /><textarea className={`${inputClass} min-h-32 sm:col-span-2`} name="bio" placeholder="بیوگرافی" defaultValue={item?.bio || ""} /><input className={inputClass} name="github" type="url" placeholder="لینک GitHub" defaultValue={item?.github || ""} /><input className={inputClass} name="linkedin" type="url" placeholder="لینک LinkedIn" defaultValue={item?.linkedin || ""} /><input className={inputClass} name="twitter" type="url" placeholder="لینک X/Twitter" defaultValue={item?.twitter || ""} /><input className={inputClass} name="sort_order" type="number" min="0" defaultValue={item?.sort_order ?? 0} placeholder="ترتیب" /></>}
...
export default function AdminDashboard() {
  ...
  return (
    <main dir="rtl" className="fixed inset-0 z-[100] overflow-hidden bg-[#edf4f0] text-right text-slate-800" style={{ backgroundImage: "radial-gradient(circle at 10% 5%, rgba(16,185,129,.12), transparent 28%), radial-gradient(circle at 80% 100%, rgba(20,184,166,.08), transparent 32%)" }}>
      ...
      <div className="h-full overflow-y-auto lg:mr-72">
        ...
        <div className="mx-auto max-w-[1500px] p-4 sm:p-7">
          ...
          {active === "projects" && <section className="space-y-5">
            ...
            {!filteredProjects.length && <div className="rounded-3xl bg-white py-24 text-center text-slate-400">پروژه‌ای پیدا نشد.</div>}
          </section>}
      ...
      {createType && <CreatePanel type={createType} item={editingItem} apiKey={apiKey} onClose={closeEditor} onCreated={() => loadData()} />}
